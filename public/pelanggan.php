...
<body>
  ...
  if (isset($_GET['edit'])) {
    $id = $_GET['edit'];
    $dataedit = mysqli_fetch_assoc(mysqli_query($conn, "SELECT * FROM pelanggan WHERE id_member = $id"));
                ...
                <tbody>
                  ...
                  foreach ($data as $pelanggan) : ?>
                    <tr>
                      ...
                      <td>
                        <a class="btn btn-sm btn-warning" href="pelanggan.php?edit=<?= $pelanggan['id_member'] ?>"><i class="icon icon-l cil-pencil"></i></a>
                        <a class="btn btn-sm btn-danger" href="../process/hapus.php?s=pelanggan&id=<?= $pelanggan['id_member'] ?>" onclick="return confirm('Hapus data?')"><i class="icon icon-l cil-trash"></i></a>
                      </td>
                    </tr>
                  <?php endforeach; ?>
                </tbody>
    ...
    <div class="modal fade" id="edit" tabindex="-1" aria-labelledby="modalLabel" aria-hidden="true">
      <div class="modal-dialog">
        <div class="modal-content">
          ...
          <div class="modal-body">
            <form class="needs-validation" action="../process/edit.php" method="post" id="editpelanggan" novalidate>
          ...
          <div class="modal-footer">
            <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Close</button>
            <button type="submit" class="btn btn-primary" name="editpelanggan" form="editpelanggan">Simpan</button>
          </div>
        </div>
      </div>
    </div>
